  ...
  return <section className="space-y-5">
    ...
    {detail && <article className="border rounded p-5 space-y-4" key={`${uid}:${revision}`}>
      ...
      {canGlobal && <div className="flex gap-4 flex-wrap">
        ...
        {detail.account_status === 'ACTIVE' && detail.providers.includes('NATIVE') && <button disabled={busy} onClick={() => confirm('Force password change? All current native tokens will be revoked. Password-change completion is not yet available; this user will remain blocked until that flow is available.', () => mutate(`/platform/users/${uid}/force-password-change`, 'POST'))}>Force password change</button>}
      </div>}
