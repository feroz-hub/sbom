...
- **Stage banding:** `001-009` ingress · `010-019` format · `020-039` schema · `040-069` semantic · `070-079` cross-ref · `080-099` security · `100-109` NTIA · `110-119` signature · `120-129` deduplication.
...
| `SBOM_VAL_E043_LICENSE_EXPRESSION_INVALID` | 422 | error | License expression '{value}' at {path} is unparseable: {reason}. — or — License expression '{value}' at {path} uses unknown licence key(s): {keys}. | Use an SPDX License List identifier, or declare a user-defined licence as `LicenseRef-<id>` with a matching `hasExtractedLicensingInfos` entry. See https://spdx.dev/learn/handling-license-info/. | SPDX 2.3 Annex D |
...
## Stage 9 — Component Deduplication (W120–W129)

| Code | HTTP | Severity | Message template | Remediation | Spec ref |
|---|---|---|---|---|---|
| `SBOM_VAL_W120_DUPLICATE_COMPONENT_DETECTED` | 422 | warning | Duplicate component detected with key '{key}'. First occurrence was at components[{first_idx}]. | Merge duplicate component definitions into a single unique component definition. | — |
...
| `SBOM_VAL_W120_DUPLICATE_COMPONENT_DETECTED` | 422 | `warning` | 9 deduplication |

_Generated by `scripts/gen_error_code_reference.py` from `app/validation/errors.py`. 66 codes total._
