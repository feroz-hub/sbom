...
def _short(text: Optional[str], n: int = 110) -> str:
...
def _flatten_ghsa(run: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for comp in run.get("components") or []:
        ...
        cver = comp.get("version", "")
        purl = comp.get("purl")
        advisories = comp.get("advisories") or (comp.get("sources", {}).get("ghsa", {}).get("advisories") if comp.get("sources") else [])
...
def _flatten_osv(run: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for comp in run.get("components") or []:
        ...
        cver = comp.get("version", "")
        purl = comp.get("purl")
        advisories = comp.get("advisories") or (comp.get("sources", {}).get("osv", {}).get("advisories") if comp.get("sources") else [])
        for v in advisories or []:
            rows.append({
                "component": cname,
                "version": cver,
...
def build_pdf_from_run_bytes(run: Dict[str, Any], title: str = "SBOM Vulnerability Report") -> bytes:
    ...
    styles.add(ParagraphStyle(
        ...
        textColor=colors.black, spaceBefore=12, spaceAfter=6, keepWithNext=True
    ))

    # Metadata labels/values
    styles.add(ParagraphStyle(
        name="MetaKey", parent=styles["Normal"],
        fontName="Helvetica-Bold", fontSize=10, leading=12,
        textColor=MUTED
    ))
    styles.add(ParagraphStyle(
        name="MetaVal", parent=styles["Normal"],
    ...
    meta_rows = [
        [Paragraph(f"{escape(k)}", styles["MetaKey"]), Paragraph(escape(v), styles["MetaVal"])]
        for k, v in meta
    ]
    ...
    for row_idx, sev in severity_by_row.items():
        ...
        base_style.append(("TEXTCOLOR", (2, row_idx), (2, row_idx),
                           colors.white if sev in ("CRITICAL", "HIGH") else colors.black))
