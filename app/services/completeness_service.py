...
def validate_completeness(sbom_json: dict[str, Any]) -> dict[str, Any]:
    ...
    if not has_dependencies:
        document_warnings.append("No dependency relationship declared (CycloneDX `dependencies[]` or SPDX `relationships[]` is missing or empty).")
    ...
    for i, c in enumerate(components):
        ...
        if is_cyclonedx:
            ...
            has_license = bool(
                (concluded and concluded != "NOASSERTION" and concluded != "NONE") or
                (declared and declared != "NOASSERTION" and declared != "NONE")
            )
...
def compute_and_save_completeness(db: Session, sbom: SBOMSource) -> dict[str, Any]:
    ...
    if not sbom.sbom_data:
        return {
            ...
            "missing_fields": [],
            "document_warnings": ["No SBOM data stored."]
        }
