...
def test_post_compare_returns_full_payload(client, db):
    ...
    for k in (
        ...
        "fix_available_pct_a",
        "fix_available_pct_b",
        "fix_available_pct_delta",
        "high_critical_count_a",
        "high_critical_count_b",
...
def test_post_compare_400_on_same_run(client, db):
    ...
    resp = client.post("/api/v1/compare", json={"run_a_id": run.id, "run_b_id": run.id})
...
def test_post_compare_404_on_missing_run(client, db):
    ...
    resp = client.post("/api/v1/compare", json={"run_a_id": run.id, "run_b_id": 999999})
...
def test_post_compare_409_on_running_run(client, db):
    ...
    resp = client.post("/api/v1/compare", json={"run_a_id": run_a.id, "run_b_id": run_b.id})
...
def test_export_cycle_markdown_csv_json(client, db):
    ...
    # Markdown
    md = client.post(f"/api/v1/compare/{cache_key}/export", json={"format": "markdown"})
    assert md.status_code == 200
    assert md.headers["content-type"].startswith("text/markdown")
    assert b"# Compare:" in md.content

    # CSV
    csvr = client.post(f"/api/v1/compare/{cache_key}/export", json={"format": "csv"})
    ...
    # JSON round-trip
    jsr = client.post(f"/api/v1/compare/{cache_key}/export", json={"format": "json"})
...
def test_export_404_on_unknown_cache_key(client):
    bogus = "0" * 64
    resp = client.post(f"/api/v1/compare/{bogus}/export", json={"format": "json"})
    assert resp.status_code == 404
    assert resp.json()["detail"]["error_code"] == "COMPARE_E006_CACHE_MISS"


def test_export_400_on_malformed_cache_key(client):
    resp = client.post("/api/v1/compare/not-hex/export", json={"format": "json"})
    assert resp.status_code == 400
