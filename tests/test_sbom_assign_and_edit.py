...
def test_validation_repair_import_preserves_selected_project(client):
    ...
    patch_session_resp = client.patch(f"/api/sbom-validation-sessions/{session_id}", json={"project_id": project["id"]})
    ...
    # Import
    imported = client.post(f"/api/sbom-validation-sessions/{session_id}/import", params={"project_required": True})
