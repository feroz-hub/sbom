...
def test_report_migrations_round_trip_preserves_inventory(client):
    ...
    with engine.connect() as conn:
        assert conn.execute(text("SELECT version_num FROM alembic_version")).scalar_one() == "053_tenant_analysis_schedule"
        assert conn.execute(text("SELECT project_name FROM projects WHERE id=:id"), {"id": identifier}).scalar_one() == "migration-preservation"
    indexes = {item["name"]: item for item in inspect(engine).get_indexes("report_subscription")}
    assert all(indexes[f"uq_report_subscription_{scope}"]["unique"] for scope in ["tenant", "project", "product", "sbom"])
